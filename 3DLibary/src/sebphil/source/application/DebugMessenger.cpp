...
namespace glib {
	...
	void APIENTRY DebugMessenger::standardMsgCallback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar* message, const void* userParam) {
	...
	std::string DebugMessenger::getSourceStr(std::uint32_t source) {
	...
	std::string DebugMessenger::getTypeStr(std::uint32_t type) {
	...
	std::string DebugMessenger::getSeverityStr(std::uint32_t severity) {
	...
	void DebugMessenger::printMessage(const Message& msg) {
	...
	DebugMessenger::DebugMessenger(SEB_GL_MSG_CALLBACK_FUNC_PTR) {

		setGlMessageCallback(this->glMessageCallback);
	}

	void DebugMessenger::setGlMessageCallback(SEB_GL_MSG_CALLBACK_FUNC_PTR) {
